/**
 * Bling Products ETL Endpoint
 * Handles product synchronization from Bling ERP
 */
...
/**
 * POST /api/etl/bling/products
 * Sync products from Bling ERP
 */
export async function POST(request: NextRequest) {
  const timer = createTimer('bling_products_etl_endpoint');
  ...
  try {
    ...
    logSecure('info', 'Bling products ETL started', { 
      companyId, 
      userId,
      endpoint: '/api/etl/bling/products'
    });

    // Run ETL process
    const result = await etlService.syncProducts(companyId);
    ...
    if (result.success) {
      logSecure('info', 'Bling products ETL completed successfully', {
        companyId,
        pages: result.pages,
        rows: result.rows,
        duration: result.duration,
      ...
      logSecure('error', 'Bling products ETL failed', {
        companyId,
        error: result.error,
        duration: result.duration,
      });
    ...
    logSecure('error', 'Bling products ETL endpoint failed', {
