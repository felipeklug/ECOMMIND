/**
 * Shopee Health Check API
 * GET /api/shopee/health
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get Shopee integration for this company
    ...
    if (integrationError || !integration) {
      return NextResponse.json({
        ...
        message: 'Shopee integration not configured',
        connected: false,
        lastSync: null,
        errors: [],
      }, { status: 200 });
    ...
    try {
      ...
      const syncStatus = {
        ...
        stock: integration.last_sync_stock,
      ...
      logSecure('Shopee health check completed', {
        companyId: context.companyId,
        status: healthResult.status,
        tokenValid: healthResult.tokenValid,
        shopId: healthResult.shopId,
      ...
      return NextResponse.json({
        ...
        stats: {
          ...
          itemMappings: mappingsCount || 0,
        },
        lastCheck: healthResult.lastCheck,
      });

    } catch (tokenError) {
      logSecure('Shopee health check failed - token error', {
        companyId: context.companyId,
    ...
    logSecure('Shopee health check API error', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });

    return NextResponse.json({
      error: 'Internal server error',
      message: 'Failed to check Shopee integration health',
    }, { status: 500 });
  }
}
